...
// Function to start autoplay
    function startAutoplay() {
        ...
        // Set new interval that calls navigate with NEXT direction
...
// Initialize all slides - make sure only the first one is visible
    function initSlides() {
...
// Call initSlides during initialization
    initSlides();

// Update performNavigation to prepare next slide before animation
    function prepareNextSlide(nextSlide, nextImage) {
        // Make the next slide visible but with opacity 0
        gsap.set(nextSlide, {
            visibility: 'visible',
            opacity: 0,
            zIndex: 1
        ...
        // Current slide should be on top during transition
        gsap.set(slides[currentIndex], { zIndex: 2 });
    }

// Start autoplay when page loads
    startAutoplay();

// Optional: pause autoplay when user interacts with slider
    document.querySelector('.slider-container').addEventListener('mouseenter', () => {
        stopAutoplay();
    });

// Optional: resume autoplay when user's mouse leaves the slider
