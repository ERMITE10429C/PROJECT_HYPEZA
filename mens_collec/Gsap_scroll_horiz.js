document.addEventListener("DOMContentLoaded", () => {
    ...
    let lastFrameTime = 0;
    const frameThrottle = 1000/60; // Target 60fps
    ...
    verticalSection.style.transform = "translateY(0)";
    ...
    const handleWheel = (e) => {
        ...
        if (currentPanel === PANEL_COUNT - 1 && e.deltaY > 0) {
            // Calculate how close we are to the end (as a percentage)
            const scrollProgress = currentX / maxScroll;

            // Once we're at least 75% through the last panel, start allowing vertical scrolling
            if (scrollProgress > 0.99) {
                // Unlock page for vertical scrolling
                document.body.style.overflow = "auto";
                return; // Exit the function to allow natural scrolling
            }
        }
    ...
    function handleScroll() {
        // Only run when horizontal scrolling is complete
        if (currentPanel >= PANEL_COUNT - 1) {
            // Calculate how much the user has scrolled past the horizontal section
            const scrollTop = window.scrollY;
            const horizontalHeight = horizontalContainer.offsetHeight;

            if (scrollTop > horizontalHeight - window.innerHeight) {
                // User has scrolled past the horizontal section
                // Enable vertical scrolling for the products grid
                document.body.style.overflow = "auto";
            } else {
                // User is still in horizontal section
                // Lock to horizontal scrolling
                window.scrollTo(0, 0);
            }
        }
    }
