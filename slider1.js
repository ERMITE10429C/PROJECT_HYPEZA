document.addEventListener("DOMContentLoaded", () => {
    ...
    const sliderContainers = document.querySelectorAll('.cloneable');
    ...
    function initSlider(container) {
        ...
        const slides = gsap.utils.toArray('[data-slider="slide"]', container);
        ...
        totalElement.textContent = formatNumber(totalSlides);

        // Create step elements dynamically
        stepsParent.innerHTML = ''; // Clear existing steps
        slides.forEach((_, index) => {
            const stepClone = stepElement.cloneNode(true);
            stepClone.textContent = formatNumber(index + 1);
            stepsParent.appendChild(stepClone);
        });

        // Get all dynamically generated steps
        ...
        const loop = horizontalLoop(slides, {
            ...
            onChange: (element, index) => {
                ...
                // Add active class to next element (design is offset)
                const nextSibling = element.nextElementSibling || slides[0];
                nextSibling.classList.add("active");
                activeElement = nextSibling;

                // Update step indicator position
                gsap.to(allSteps, {
                    y: `${-100 * index}%`,
                    ease: "power3",
                    duration: 0.45
        ...
        const slideTransition = {ease: "power3", duration: 0.725};

        // Set up navigation
        slides.forEach((slide, i) => {
            slide.addEventListener("click", () => loop.toIndex(i - 1, slideTransition));
        });

        nextButton.addEventListener("click", () => loop.next(slideTransition));
        prevButton.addEventListener("click", () => loop.previous(slideTransition));
    }
